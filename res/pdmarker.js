...
var cr = "<br/>";
var s =
"1.99f 07/09/06 - (unreleased) zoomToMarkers now takes into account markers not displayed." + cr +
...
google.maps.Map.prototype.getNthMarker = function(nTh) {
	var count = getPdMarkerCount(this);
	for (var i = 0; i < count; i++)
		if (isPdMarker(this.pdMarkers[i]))
		{
...
google.maps.Map.prototype.boxMap = function(center, span) {
	...
	this.centerAndZoom(new google.maps.Point(center.x, center.y), zoom);
}
...
google.maps.Map.prototype.zoomToMarkers = function(slopPercentage, heightOffsetPct) {
	...
	else if (count > 1)
	{
		...
		span = new google.maps.Size(Math.abs(maxX - minX), Math.abs(maxY - minY));
		slopWid = 0;
		slopHgt = 0;
...
PdMarker.prototype = new google.maps.Marker(new google.maps.LatLng(1, 1));
...
PdMarker.prototype.initialize = function(a) {
	if (typeof a == "google.maps.Map")
	{
		console.error("PdMarker requires google.maps.Map");
		return;
	}
