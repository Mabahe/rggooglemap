...
//		debug($PA);
		...
		if($config['treeView'] AND $config['foreign_table']) {
			...
			if($config['treeViewClass'] AND is_object($treeViewObj = &t3lib_div::getUserObj($config['treeViewClass'],'user_',false)))      {
				...
				$treeViewObj = t3lib_div::makeInstance('tx_rggm_tceFunc_selectTreeView');
			}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/rggooglemap/lib/class.tx_rggooglemap_treeview.php']) {
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/rggooglemap/lib/class.tx_rggooglemap_treeview.php']);
}
