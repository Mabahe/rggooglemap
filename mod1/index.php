...
class tx_rggooglemap_module1 extends t3lib_SCbase {
	...
	function main() {
		...
		if (($this->id && $access) || ($GLOBALS['BE_USER']->user["admin"] && !$this->id)) {
			...
			$this->content .= '<div style="color:red;text-align:center;font-weight:bold;">' . $GLOBALS['LANG']->getLL("noPageChosen") . '</div>';
			$this->content .= $this->doc->spacer(10);
		}
	...
	function viewRecordList() {
		...
		$content = $this->doc->section($GLOBALS['LANG']->getLL("headerList"), '', 0, 1);
		...
		$res = $this->generic->exec_SELECTquery($field, $this->confArr['tables'], $where);
		...
		if (count($res) != 0) { # at lease one record available
			$out = '
             ...
             <div style="height:400px;overflow:auto;"><table cellpadding="1" cellspacing="1" class="bgColor4 sortable" id="listrecord" width="100%">
              <tr class="tableheader bgColor5">
                <td></td>
                <td>' . $GLOBALS['LANG']->getLL("name") . '</td>
                <td>' . $GLOBALS['LANG']->getLL("category") . '</td>
                <td align="center">' . $GLOBALS['LANG']->getLL("table") . '</td>
                <!--<td align="center">' . $GLOBALS['LANG']->getLL("display") . '</td>--!>
                <td align="center">' . $GLOBALS['LANG']->getLL("map") . '</td>
              </tr>';
			...
			while ($row = array_shift($res)) { # single record
				...
				if (intval($row['lng']) == 0 || intval($row['lat']) == 0) {
					...
					$hide = '';
					$map = '<a style="cursor: help;" onclick="loadhover(\'' . $title . '\',' . $row['lat'] . ',' . $row['lng'] . ')">' . $GLOBALS['LANG']->getLL("mapLink") . '</a>';
				}
			...
			$out .= '</table></div>
              ...
              <div id="mapLink" style="visibility:hidden;" class="right">
								<a href="javascript:ShowHide(\'mapHover\')">' . $GLOBALS['LANG']->getLL("showHideMap") . '</a>
							</div>
             <!--<fieldset><legend>' . $GLOBALS['LANG']->getLL("headerListSave") . '</legend>
             <input type="checkbox" name="control" id="control" value="1" /> <label for="control">' . $GLOBALS['LANG']->getLL("confirmation") . '</label>
             <br />
             <div class="right"><input id="submitButton" type="button" value="' . $GLOBALS['LANG']->getLL("save") . '" onclick=\'xajax_listRecords(xajax.getFormValues("listForm"));\' /></div>
             </fieldset>--!></form>';
		}
	...
	function xajaxListRecords($arg) {
		...
		if ($arg['control'] == 1) {
			...
			$content = '<div class="success">' . $GLOBALS['LANG']->getLL("saveDisplay") . '</div>';
		} else {
			$content = '<div class="error">' . $GLOBALS['LANG']->getLL("saveErrorCheckbox") . '</div>';
		}
	...
	function viewMap() {
		$content = $this->doc->section($GLOBALS['LANG']->getLL("headerChoosePOI"), '', 0, 1);
		$selection = '';

		if ($this->wizardMode) {
			$content .= $GLOBALS['LANG']->getLL("latitude") . ': <input id="centerlatitude" name="google[centerlatitude]" value="' . $this->confArr['startLat'] . '" type="text"> &nbsp; &nbsp;' .
				$GLOBALS['LANG']->getLL("longitude") . ': <input id="centerlongitude" name="google[centerlongitude]" value="' . $this->confArr['startLong'] . '" type="text"> &nbsp; &nbsp;' .
				$GLOBALS['LANG']->getLL("zoom") . ': <input id="centerzoom"  style="width:20px" disabled="disabled" value="' . $this->confArr['startZoom'] . '" type="text" />  &nbsp; &nbsp;
					<input id="submitButton" type="button" value="' . $GLOBALS['LANG']->getLL("save") . '" onclick=\'savePoint("map' . $this->mapIdx . '");\' /> &nbsp; &nbsp;
					<input id="submitButton" type="button" value="' . $GLOBALS['LANG']->getLL("save_and_close") . '" onclick=\'savePoint("map' . $this->mapIdx . '"); window.close();\' /> &nbsp; &nbsp;
					<input id="submitButton" type="button" value="' . $GLOBALS['LANG']->getLL("close") . '" onclick=\'window.close();\' /><br />

                  <div id="options">
                    <form action="#" onsubmit="showAddress(this.address.value); return false">
                      <fieldset>
                        <legend>' . $GLOBALS['LANG']->getLL("headerGeocode") . '</legend>
                        <input type="text" size="70" name="address" value="' . $GLOBALS['LANG']->getLL("geocodeSearchValue") . '" onfocus="if(this.value==\'' . $GLOBALS['LANG']->getLL("geocodeSearchValue") . '\')this.value=\'\';" />
                        <input type="submit" value="' . $GLOBALS['LANG']->getLL("geocodeButton") . '"  />
                      </fieldset>
                    </form>
			...
			$content .= '
                  ...
                  <form id="testForm2" name="testForm2">
                      ' . $GLOBALS['LANG']->getLL("latitude") . ': <input id="centerlatitude" name="google[centerlatitude]" value="' . $this->confArr['startLat'] . '" type="text">
                      ' . $GLOBALS['LANG']->getLL("longitude") . ': <input id="centerlongitude" name="google[centerlongitude]" value="' . $this->confArr['startLong'] . '" type="text">
                      <input type="hidden" name="google[step2]" value="1">
                    <br />
                    <input id="submitButton" type="button" value="' . $GLOBALS['LANG']->getLL("save") . '" onclick=\'xajax_getPoi(xajax.getFormValues("testForm2"));\' />
                  </form>
                  <br />

                  <a href="javascript:ShowHide(\'options\')">' . $GLOBALS['LANG']->getLL("showHideOptions") . '</a>
                  <div id="options" style="visibility:hidden;height:0px;">
                    ...
                    <form id="loadForm" name="loadForm">' . $selection . '
                      <fieldset>
                        <legend>' . $GLOBALS['LANG']->getLL("headerloadRecord") . '</legend>
                    ...
                    <fieldset>
                      <legend>' . $GLOBALS['LANG']->getLL("headerloadPOI") . '</legend>
                      <a href="#" onClick="loadPOI()">' . $GLOBALS['LANG']->getLL("loadPOILink") . '</a>
                    </fieldset>
                  </div>
	...
	function xajaxGetPoi($arg) {
		...
		if ($arg['centerlongitude'] != '') {
			...
			$content = '<br /><div class="error">' . $GLOBALS['LANG']->getLL("saveError") . '</div>';
		}
	...
	function getPOIs($arg) {
		$content = $this->doc->section($GLOBALS['LANG']->getLL("headerSave"), '', 0, 1);

		// 1) inserts POI to a new record
		$table = 'tt_address';
		$content .= '<h4>' . $GLOBALS['LANG']->getLL("headerSavePOINew") . '</h4><ul>';
		...
		$content .= '<h4>' . $GLOBALS['LANG']->getLL("headerSavePOI") . '</h4>';
		...
		$content .= '
                <form id="saveForm" name="saveForm">' . $selection . '
                  ...
                  <input type="checkbox" name="override" id="override" value="on" /> <label for="override">' . $GLOBALS['LANG']->getLL("confirmation") . '</label>
                  <br /><br />
                  <input id="submitButton" type="button" value="' . $GLOBALS['LANG']->getLL("save") . '" onclick=\'xajax_insertPoi(xajax.getFormValues("saveForm"));\' />
                </form>
	...
	function xajaxInsertPoi($arg) {
		...
		if ($arg['override'] == 'on' && $arg['select'] != '') {
			...
			if ($arg['select'] == '') {
				$content .= '<div class="error">' . $GLOBALS['LANG']->getLL("saveErrorDropdown") . '</div>';
			}
		}
	...
	function newSaveRecord($arg) {
		...
		while (is_object($serviceObj = t3lib_div::makeInstanceService('rggmData', $table, $serviceChain))) {
			...
			if ($tempuser = $serviceObj->init()) {
				...
				$res = $this->generic->exec_SELECTquery($field, $this->confArr['tables'], $where, '', '', '0,10');
				...
				if ($i > 0) { # double entry saveSamePosition
					$content .= '<div class="error">' . sprintf($GLOBALS['LANG']->getLL("saveSamePosition"), $doubleTitle) . '</div>';
				} else {
					$where = 'uid = ' . $id;
					...
					$content .= '<div class="success">' . $GLOBALS['LANG']->getLL("saveSuccss") . '</div>';
				}
				return $content;
			}
		}
	...
	function xajaxAutoComplete($arg) {
		...
		if (strlen($autoselect) > 0 || $autoselect == '*') {
			...
			if ($autoselect != '*') {
				$where .= 'AND name LIKE \'%' . $autoselect . '%\'';
			}
	...
	function viewMassGeocoding() {
		...
		if ($vars['start'] == 1 && count($vars['tables']) > 0) {
			...
			$res = $this->generic->exec_SELECTquery($field, $tables, $where);
	...
	function getIconFromRecord($table, $row) {
		...
		$elementTitle = t3lib_div::fixed_lgd_cs($elementTitle, -($GLOBALS['BE_USER']->uc['titleLen']));
		$elementIcon = t3lib_iconworks::getIconImage($table, '', $GLOBALS['BACK_PATH'], 'class="c-recicon" title="' . $iconAltText . '"');
	...
	function getIconFromRecordNoLink($table) {
		$elementIcon = t3lib_iconworks::getIconImage($table, '', $GLOBALS['BACK_PATH'], '');
		return $elementIcon;
	...
	function getNewRecord($table, $pid, $lngVal, $latVal) {
		...
		if ($latField != '' && $lngField != '') {
			...
			$out = '<li><a href="#" onclick="' . htmlspecialchars(t3lib_BEfunc::editOnClick($params, $GLOBALS['BACK_PATH'])) . '">' . '<b>' . $table . '</b>: ' . $GLOBALS['LANG']->getLL("clickHere") . '</a></li>';
		}
	...
	function getCatTitle() {
		...
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('uid, title', 'tx_rggooglemap_cat', $where);
	...
	function getCatColor() {
		$where = 'hidden = 0 AND deleted = 0';
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('uid, color', 'tx_rggooglemap_cat', $where);
		while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
			if ($row['color'] != '') {
				$data [$row['uid']] = '<div style="background:' . $row['color'] . ';" class="catcolor"></div>';
			}
		}
		$GLOBALS['TYPO3_DB']->sql_free_result($res);
		return $data;
...
if (defined('TYPO3_MODE') && isset($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/rggooglemap/mod1/index.php'])) {
	include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/rggooglemap/mod1/index.php']);
}

if (!$GLOBALS['SOBE']) {
	/** @var $SOBE tx_rggooglemap_module1 */
