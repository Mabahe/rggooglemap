...
function makeMap() {
	if (true /*GBrowserIsCompatible()*/) {
		...
		map = new google.maps.Map(document.getElementById("map") ###MAP_TYPES###);
		...
		map.setCenter(new google.maps.LatLng(###MAP_LAT###, ###MAP_LNG###), ###MAP_ZOOM###);
...
function myclick(i, lng, lat, table,showMarker) {
	...
	req.onreadystatechange = function() {
		if (req.readyState == 4) {
			t.map.openInfoWindowHtml( new google.maps.LatLng(lat, lng), req.responseText );
		}
	};
	req.send(null);
};
